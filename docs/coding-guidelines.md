...
### Structure
- PSR-4: `AccessiWoo\` → `src/`; one class per file; class name = file name; namespace mirrors folder (`AccessiWoo\Scanner\ScanStorage`).
...
- Prefix everything `accessiwoo_` (options, hooks, transients, cron events, handles).
- Fire plugin hooks at meaningful moments (`accessiwoo_after_scan_saved`, etc.) with documented params.
- Use WP APIs over raw PHP: `wp_json_encode`, `wp_remote_*` (Pro only), `wp_mail`, `current_time`/`gmdate`, `wp_insert_post`.
- i18n: every user-facing string in `__()`/`esc_html__()`/etc. with literal text-domain `accessiwoo`; translator comments for placeholders.
...
- React via `@wordpress/element`; UI from `@wordpress/components`; i18n via `@wordpress/i18n` (`__( 'Text', 'accessiwoo' )`). These resolve to WP-core externals — never bundle React itself.
...
- Files per §5.2: `admin.css`, `frontend-fixes.css`. All selectors prefixed `.accessiwoo-`.
- `frontend-fixes.css`: minimal, defensive specificity, scoped under the `body_class` flags (`.accessiwoo-fixes-active`); must never restyle theme elements beyond the fix's stated purpose.
...
- Version bumps: plugin header + `ACCESSIWOO_VERSION` + `readme.txt` stable tag + changelog together.
