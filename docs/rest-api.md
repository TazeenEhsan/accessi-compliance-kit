# REST API — Accessi Compliance Kit

Status per proposal: **optional / future**. Proposal §5.4 lists `register_rest_route( 'accessi-compliance-kit/v1', '/scans', ... )` as "REST API (optional, for future SPA admin)", and full REST endpoints are a Pro integration feature (§4.2.F: "read scans, trigger scans, fetch reports for external tooling", enabling CI/CD per §4.2.I).
...
- Namespace: `accessi-compliance-kit/v1` (proposal §5.4)
...
|---|---|---|---|
| `/accessi-compliance-kit/v1/scans` | GET | List scans (paged; mirrors History tab data) | scan read |
| `/accessi-compliance-kit/v1/scans` | POST | Trigger a scan (Pro: server-driven crawl; enables CI/CD per §4.2.I) | scan run |
| `/accessi-compliance-kit/v1/scans/{id}` | GET | One scan with full normalized violations | scan read |
| `/accessi-compliance-kit/v1/reports/{id}` | GET | Fetch a generated report (Pro, §4.2.F) | report read |
...
| Action | Purpose |
|---|---|
| `accessi_compliance_kit_run_scan` | Save posted scan results (proposal §5.4, §5.5 step 5) |
| `accessi_compliance_kit_get_scan` | Fetch one scan's stored results |
| `accessi_compliance_kit_get_scans` | Paged history list |
| `accessi_compliance_kit_save_settings` | Save settings/fix toggles (proposal §5.4) |
| `accessi_compliance_kit_generate_statement` | Create the statement page |
...
- **WP-CLI** (§4.2.F): `wp accessi-compliance-kit scan`, `wp accessi-compliance-kit report` — same service layer, enables staging scans on deploy.
