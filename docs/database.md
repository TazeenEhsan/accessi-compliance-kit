...
## 1. Custom Table: `{$wpdb->prefix}accessiwoo_scans`
...
- Store the schema version in the `accessiwoo_db_version` option; on upgrade, `Plugin` compares and re-runs `dbDelta()` when it changes.

## 2. Scan Row Lifecycle

1. `create_scan( $url, $type, $user_id )` → inserts row: `status = 'running'`, `started_at = now`, empty JSON columns.
2. On successful result: `complete_scan( $id, $violations, $summary )` → `status = 'complete'`, `completed_at = now`, JSON columns filled, `accessiwoo_last_scan_id` option updated.
...
|---|---|---|
| `accessiwoo_settings` | Plugin configuration array (email opt-in, statement page ID, misc settings) | yes |
| `accessiwoo_active_fixes` | Map of fix ID → bool; which fixes are enabled. **All false by default** (proposal §9) | yes |
| `accessiwoo_license` | Pro license data (via Freemius or custom) — Phase 5 | yes |
| `accessiwoo_last_scan_id` | Pointer for the dashboard widget | yes |

Plus the internal `accessiwoo_db_version` (schema version). All option access goes through `src/Utils/Options.php` — no ad-hoc `get_option()` calls in feature code.
...
- **Uninstall** (`uninstall.php`): drop `{$wpdb->prefix}accessiwoo_scans`, delete all `accessiwoo_*` options. Guarded by `defined( 'WP_UNINSTALL_PLUGIN' )`.

## 6. Pro-Era Additions (Phase 5 — design later, don't build now)

The proposal implies future storage needs that must NOT be built in the MVP: crawl queues (§4.2.A), violation mutes/justifications (§4.2.A), remediation status + comments + activity log (§4.2.E), statement version history (§4.2.D), immutable audit history (§4.2.C). When Phase 5 starts, design these as separate tables — do not overload `wp_accessiwoo_scans`.
