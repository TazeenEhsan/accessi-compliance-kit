...
Each `wp_ajax_accessiwoo_*` handler must, in order:

1. `check_ajax_referer( 'accessiwoo_<action>', ... )` — per-action nonces localized to the admin app
...
- Scanner script itself is enqueued only when `?accessiwoo_scan=1` AND `Capabilities::can_scan()` (proposal §5.5 step 3) — anonymous requests with the flag get nothing.
...
- 100% of queries through `$wpdb->prepare()` with `%d`/`%s` placeholders; table name interpolated only from `$wpdb->prefix . 'accessiwoo_scans'`.
...
- [ ] Uninstall removes the table and all `accessiwoo_*` options
