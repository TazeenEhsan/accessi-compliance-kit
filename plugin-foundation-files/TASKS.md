...
- [x] Write `tazeen-store-accessibility-kit-for-woocommerce.php` main file: plugin headers (name, description, version, requires WP 6.5, requires PHP 7.4, license GPLv2, text domain `tazeen-store-accessibility-kit-for-woocommerce`), `ABSPATH` guard, define constants (`TSAKW_VERSION`, `TSAKW_FILE`, `TSAKW_PATH`, `TSAKW_URL`), require Composer autoloader
...
- [x] Write `src/Activator.php` stub + `src/Deactivator.php` stub; register `register_activation_hook` / `register_deactivation_hook` in `tazeen-store-accessibility-kit-for-woocommerce.php`
...
- [x] Write `src/Admin/AdminMenu.php`: register submenu page under WooCommerce → "Accessibility" (proposal §4.1), page callback renders a root `<div id="tazeen-store-accessibility-kit-for-woocommerce-admin">`
...
- [ ] End-to-end test on the dev site: scan the shop page, see grouped violations, row saved in DB, history lists it — **not yet verified**: build is clean (`npm run build`), `vendor/bin/phpunit` passes (17/17), and `/wp-admin/admin.php?page=tazeen-store-accessibility-kit-for-woocommerce` + the front page both respond without fatal errors, but no browser/WP-CLI session was available in this environment to click through the actual scan flow. Needs a manual pass in a logged-in browser.
...
- [x] Add `body_class` filter in `FixManager`: append `tazeen-store-accessibility-kit-for-woocommerce-fixes-active` (plus per-fix classes) when any fix is enabled (proposal §5.4)
...
- [x] Sweep all PHP/JS for untranslated strings; load text domain; generate `languages/tazeen-store-accessibility-kit-for-woocommerce.pot` (`wp i18n make-pot`) — one gap found (`assets/js/src/admin/utils/ajax.js`'s fallback error string) and fixed; `languages/tazeen-store-accessibility-kit-for-woocommerce.pot` generated via `wp i18n make-pot` (98 strings). Text domain was already loaded correctly from task 0.2.
