...
- [x] Sweep all PHP/JS for untranslated strings; load text domain; generate `languages/accessi-compliance-kit.pot` (`wp i18n make-pot`) — one gap found (`assets/js/src/admin/utils/ajax.js`'s fallback error string) and fixed; `languages/accessi-compliance-kit.pot` generated via `wp i18n make-pot` (98 strings). Text domain was already loaded correctly from task 0.2.
- [x] Security pass 1: verify every AJAX handler has nonce + capability + sanitization + escaped output (checklist in docs/security.md §9) — all six `wp_ajax_accessi_compliance_kit_*` handlers checked clean (nonce → capability → sanitize → service call → `wp_send_json_*`); no `wp_ajax_nopriv_*` handlers exist.
