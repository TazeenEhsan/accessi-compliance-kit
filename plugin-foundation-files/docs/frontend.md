...
- Loading rule (proposal §5.5 step 3): the bundle is enqueued on the front end **only when** the request has `?accessibility_compliance_kit_for_woocommerce_scan=1` **and** the current user passes `Capabilities::can_scan()`. Anonymous visitors never load it; there is zero front-end weight for normal traffic.
...
| `is_enabled()` | reads `accessibility_compliance_kit_for_woocommerce_active_fixes` via `Utils/Options` |
