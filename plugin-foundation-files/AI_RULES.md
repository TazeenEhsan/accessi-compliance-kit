# AI_RULES.md — Rules for AI-Assisted Development of Accessibility Compliance Kit for WooCommerce
...
- **Plugin name:** Accessibility Compliance Kit for WooCommerce (working title per proposal §1)
...
- The front-end scanner script loads **only** when `?accessibility_compliance_kit_for_woocommerce_scan=1` is present **and** the current user has the scan capability (proposal §5.5, step 3).
...
- Scan storage goes through `Scanner/ScanStorage.php`; nothing else touches `wp_accessibility_compliance_kit_for_woocommerce_scans`.
